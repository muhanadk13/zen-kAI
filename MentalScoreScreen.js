...
export default function MentalScoreScreen() {
  ...
  // Animated value for the overall MentalScore gauge
  const overallScoreAnim = useRef(new Animated.Value(-1)).current;
  const prevOverallScore = useRef(-1);
  ...
  const [displayOverallScore, setDisplayOverallScore] = useState(-1);
  const checkInButtonRef = useRef(null);

  useEffect(() => {
    const id = overallScoreAnim.addListener(({ value }) =>
      setDisplayOverallScore(Math.round(value))
    );
    return () => overallScoreAnim.removeListener(id);
  }, []);
  ...
  useEffect(() => {
    ...
    setOverallScore(target);
  }, [energy, clarity, emotion, focus]);

  useEffect(() => {
    if (overallScore < 0) return;
    if (prevOverallScore.current > -1 && overallScore < prevOverallScore.current) {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    }
    prevOverallScore.current = overallScore;
    Animated.timing(overallScoreAnim, {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Animatable.View animation="fadeInDown" duration={800} style={styles.gaugeContainer}>
        ...
        <Text style={styles.mentalScore}>{displayOverallScore}</Text>
